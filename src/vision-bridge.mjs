...
function responsesBody(engine, imageUrl, { stream = false } = {}) {
  return {
...
function describeRequest(engine, imageUrl, gatewayBase, gatewayHeaders, nativeCall) {
  if (engine.native) {
    ...
    return {
      ...
      body: responsesBody(engine, imageUrl, { stream: true }),
      stream: true,
    };
  }
  if (engine.local) {
  ...
  return {
    ...
    body: responsesBody(engine, imageUrl),
  };
}
...
export async function describeImage({
  ...
  const request = describeRequest(engine, imageUrl, gatewayBase, headers, nativeCall);
