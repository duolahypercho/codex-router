...
import { LEGACY_STATE_DIR, STATE_DIR } from "./paths.mjs";
...
export function credentialPaths(provider) {
  ...
  const candidates = names.flatMap((name) => [
    ...
    path.join(LEGACY_STATE_DIR, name),
  ]);
...
export function resolveProviderCredential(providerOrId) {
  const provider =
  ...
  for (const candidate of credentialPaths(provider)) {
    ...
    if (value) return { value, source: `protected file (${candidate})` };
  }
  return keyFromKeychain(provider);
}

export function credentialStatus(providerOrId) {
  const provider =
    typeof providerOrId === "string" ? apiProvider(providerOrId) : providerOrId;
