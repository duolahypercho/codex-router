...
// The bundled venv's pythonw.exe is the one GUI-subsystem host every install
// already carries. It starts the CMD wrapper with CREATE_NO_WINDOW, which
// hands cmd.exe a console that has no window at all, and every descendant
// (node, the forwarders, LiteLLM) inherits that same windowless console. The
// wrapper stays the single definition of the service environment, the log
// redirect, and the exit code; this file only decides how it is hosted.
function pythonHideLauncher() {
  return [
    "# Generated by Codex Router. Do not edit.",
    "# GUI-subsystem host for the Windows scheduled task: cmd.exe is a console",
    "# binary, and on Windows 11 the default terminal ignores the hidden window",
    "# style wscript.exe can ask for, so the wrapper must start with a console",
    "# that never had a window (CREATE_NO_WINDOW). Its children inherit it.",
    "import subprocess",
    "",
...
// which is a windowless host. Window style 0 (SW_HIDE) on the CMD wrapper was
// enough while conhost was the console: the wrapper owned one hidden console
// for the router's lifetime. Windows 11 with Windows Terminal as the default
// terminal ignores that style, so the same wrapper produced a persistent
// Terminal window (issue #674). A console that is created with
// CREATE_NO_WINDOW never gets a window under either host, and wscript.exe
// cannot request that flag, so the script prefers the bundled venv's
// pythonw.exe with the generated .pyw, which can. The direct cmd.exe form
// stays as the fallback for an install that carries no venv
// (MODEL_ROUTER_LITELLM_BIN); it is still hidden under conhost.
//
...
function launcher() {
  ...
  return [
    ...
    `If fso.FileExists("${vbsEscape(pythonwPath)}") And fso.FileExists("${vbsEscape(pythonLauncherPath)}") Then`,
    `  command = quote & "${vbsEscape(pythonwPath)}" & quote & " " & quote & "${vbsEscape(pythonLauncherPath)}" & quote`,
    "Else",
    `  command = "cmd.exe /D /C " & quote & quote & "${vbsEscape(wrapperPath)}" & quote & quote`,
    "End If",
...
function installTask() {
  ...
  const script = [
    ...
    // A dropped tick starts no process, and a tick that does relaunch goes
    // through the windowless launcher above, so the heartbeat draws nothing.
...
if (command === "render") {
  ...
  for (const target of [launcherPath, wrapperPath, pythonLauncherPath]) {
    try {
