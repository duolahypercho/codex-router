...
test("re-probes a legacy persisted fallback and replaces it with a managed project", async () => {
  await withToken(
    baseToken({ project_id: "rising-fact-p41fc" }),
    async () => {
      let calls = 0;
      ...
      assert.equal(result.session.project_id, "managed-project");
      assert.equal(result.session.project_source, "managed");
      assert.equal(result.session.tier_id, "pro-tier");
    },
  );
});

test("keeps the shared fallback request-only and throttles re-probes for 30 minutes", async () => {
  await withToken(baseToken(), async (_write, tokenPath) => {
    ...
    const first = await ensureAntigravityProject(readAntigravityToken(), options);
    const callsAfterFirst = calls;
    const second = await ensureAntigravityProject(readAntigravityToken(), options);
    assert.equal(first.projectId, "rising-fact-p41fc");
    assert.equal(first.source, "fallback");
    assert.equal(second.projectId, "rising-fact-p41fc");
    assert.equal(calls, callsAfterFirst);
    const raw = JSON.parse(readFileSync(tokenPath, "utf8"));
    assert.equal(raw.project_id, "");
    assert.equal(raw.project_source, "fallback");
    assert.equal(raw.project_checked_at, 20_000);
  });
});

test("forces a recent fallback and its cache to be re-probed after an upstream 401", async () => {
  await withToken(baseToken(), async () => {
    ...
    const fetchImpl = async () => {
      ...
      return new Response(JSON.stringify({ cloudaicompanionProject: "managed-after-401" }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
      });
    };
    const first = await ensureAntigravityProject(readAntigravityToken(), {
      now: () => 20_000,
      attempts: 1,
      delayImpl: async () => {},
      fetchImpl,
    ...
    phase = "managed";
    calls = 0;
    const refreshed = await ensureAntigravityProject(readAntigravityToken(), {
      now: () => 20_001,
      attempts: 1,
      delayImpl: async () => {},
      fetchImpl,
    ...
    assert.equal(calls, 1);
    assert.equal(refreshed.projectId, "managed-after-401");
    assert.equal(refreshed.session.project_source, "managed");
  });
});
