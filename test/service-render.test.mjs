...
test("the Windows launcher prefers a windowless pythonw host and falls back to the hidden wrapper", () => {
  ...
  try {
    ...
    // Windows Terminal ignores the hidden window style (issue #674), so the
    // launcher prefers the bundled venv's GUI-subsystem pythonw.exe, which
    // starts the wrapper with CREATE_NO_WINDOW. Both files are checked: a
    // missing venv (MODEL_ROUTER_LITELLM_BIN installs) or a launcher written
    // by an older version must still start the router.
    assert.ok(
      script.includes(
        `If fso.FileExists("${pythonwPath}") And fso.FileExists("${pythonLauncherPath}") Then\r\n`
        + `  command = quote & "${pythonwPath}" & quote & " " & quote & "${pythonLauncherPath}" & quote\r\n`
        + "Else\r\n"
        + `  command = "cmd.exe /D /C " & quote & quote & "${wrapperPath}" & quote & quote\r\n`
        + "End If\r\n",
      ),
      `launcher did not embed the pythonw host and the wrapper fallback correctly:\n${script}`,
    );
    // Window style 0 hides the wrapper's console under conhost; True waits for
    // it so that Run returns the wrapper's exit code instead of returning
    // immediately. LastTaskResult still needs the real exit code for
    // doctor/readiness.
    ...
    // The .pyw is the only place CREATE_NO_WINDOW can be requested. It hosts
    // the wrapper rather than node directly so the service environment, the
    // log redirect, and the exit code keep exactly one definition.
    const python = serviceCommand("service-windows.mjs", "win32", testRoot, "render-python");
    assert.match(python, /^# Generated by Codex Router\. Do not edit\.\n/);
    assert.match(python, /CREATE_NO_WINDOW = 0x08000000\n/);
    assert.ok(python.includes(`WRAPPER = ${JSON.stringify(wrapperPath)}\n`));
    assert.match(python, /\['cmd\.exe', '\/D', '\/C', WRAPPER\],\n\s+creationflags=CREATE_NO_WINDOW,\n\s+stdin=subprocess\.DEVNULL,/);
    assert.doesNotMatch(python, /start\.mjs/);
    assert.doesNotMatch(python, /\r/);
    const interpreter = ["python3", "python"]
      .map((name) => spawnSync(name, ["-c", "import sys; sys.exit(0)"], { stdio: "ignore", windowsHide: true }))
      .findIndex((result) => !result.error && result.status === 0);
    if (interpreter !== -1) {
      // A generated Python file that does not parse would strand every
      // scheduled start on the wrapper fallback without a diagnostic.
      const compiled = spawnSync(
        ["python3", "python"][interpreter],
        ["-c", "import ast, sys; ast.parse(sys.stdin.read())"],
        { input: python, encoding: "utf8", windowsHide: true },
      );
