...
final class RouterStore: ObservableObject {
  ...
  init() {
    ...
    if let raw = defaults.string(forKey: islandModeKey), let mode = IslandMode(rawValue: raw) {
      ...
      // Keep the menu-bar panel as the first-run surface. The notch overlay is
      // opt-in so a fresh install cannot cover the user's active window.
      islandMode = .off
    } else {
    ...
    HStack(spacing: 12) {
      VStack(alignment: .leading, spacing: 3) {
        ...
        Text(store.islandMode == .desktop
          ...
          : routerLocalized("Show provider usage and activity status"))
          .font(.system(size: 10))
          .foregroundStyle(routerMuted)
      }
      Spacer()
